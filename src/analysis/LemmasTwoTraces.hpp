...
/*
 * We use the lemmas in this header to cover the inductive reasoning which
 * relates values and timepoints of two execution-traces. Each of these lemmas
 * is added for any pair (t1,t2) of (different) traces.
 */

/*
 * LEMMA 1
 * If a variable has the same value (or if an array has at one position the same
 * value) in both traces at some iteration, and if that equality is preserved in
 * an interval, then the equality holds during any point of the interval
 * (including the end of the interval).
 *
 * Soundness: This lemma is exactly the instantiation of the following standard
 * induction axiom scheme with P(it) := v(l(it),t1)=v(l(it),t2) forall
 * boundL,boundR.
 *    =>
 *       and
 *          P(boundL)
 ...
 *
 * Variations:
 *  The currently used version is the most general one, and allows arbitrary
 * intervals. More specific versions could fix the left bound to 0 and/or the
 * right bound to the last iteration of one trace (wlog. trace 1).
 *
 * Why is this lemma useful
 *  This lemma is central for reasoning about relational properties.
 */
class EqualityPreservationTracesLemmas
