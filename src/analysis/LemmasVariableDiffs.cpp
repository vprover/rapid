...
namespace analysis {
    ...
    void VariableDiffLemmas::generateOutputFor(const program::WhileStatement *statement, std::vector<std::shared_ptr<const logic::ProblemItem>>& items)
    {
        ...
        for (const auto& v : locationToActiveVars.at(locationSymbolForStatement(statement)->name))
        {
            ...
            for (unsigned traceNumber = 1; traceNumber < numberOfTraces+1; traceNumber++)
            {
                ...
                if(diff){
                    ...
                    //1 * n = n
                    auto diffTermXMaxLoops = diff == 1 ? toIntN :
    ...
    void VariableDiffLemmas::addToIntAxioms(std::vector<std::shared_ptr<const logic::ProblemItem>>& items)
    {
        ...
        auto injectivityAxiom =
            logic::Formulas::universal({natSym1, natSym2},
                logic::Formulas::implication(
                    logic::Formulas::equality(n1, n2),
                    logic::Formulas::equality(
                        logic::Theory::toInt(n1), 
