...
namespace analysis {
    ...
    void SemanticsInliner::computePersistentTermsRec(std::shared_ptr<const logic::Term> t) {
        switch (t->type()) {
            case logic::Term::Type::Equality: {
                auto castedFormula = std::static_pointer_cast<const logic::EqualityFormula>(t);
            ...
            case logic::Term::Type::Conjunction: {
                auto castedFormula = std::static_pointer_cast<const logic::ConjunctionFormula>(t);
            ...
            case logic::Term::Type::Disjunction: {
                auto castedFormula = std::static_pointer_cast<const logic::DisjunctionFormula>(t);
            ...
            case logic::Term::Type::Negation: {
                auto castedFormula = std::static_pointer_cast<const logic::NegationFormula>(t);
                computePersistentTermsRec(castedFormula->f);
                break;
            }
            case logic::Term::Type::Existential: {
                auto castedFormula = std::static_pointer_cast<const logic::ExistentialFormula>(t);
                computePersistentTermsRec(castedFormula->f);
                break;
            }
            case logic::Term::Type::Universal: {
                auto castedFormula = std::static_pointer_cast<const logic::UniversalFormula>(t);
                computePersistentTermsRec(castedFormula->f);
                break;
            }
            case logic::Term::Type::Implication: {
                auto castedFormula = std::static_pointer_cast<const logic::ImplicationFormula>(t);
                computePersistentTermsRec(castedFormula->f1);
                computePersistentTermsRec(castedFormula->f2);
                break;
            }
            case logic::Term::Type::Equivalence: {
                auto castedFormula = std::static_pointer_cast<const logic::EquivalenceFormula>(t);
                computePersistentTermsRec(castedFormula->f1);
                computePersistentTermsRec(castedFormula->f2);
                break;
            }
            ...
            case logic::Term::Type::FuncTerm: {
                ...
                if (
                        castedTerm->symbol->rngSort == logic::Sorts::intSort() ||
    ...
    std::shared_ptr<const logic::Term> SemanticsInliner::toCachedTerm(std::shared_ptr<const program::Expression> expr) {
        ...
        if (typeid(*expr) == typeid(program::ArithmeticConstant)) {
            ...
            return logic::Formulas::disjunction({toCachedTerm(castedExpr->child1), toCachedTerm(castedExpr->child2)});
        }
    ...
    std::shared_ptr<const logic::Term> InlinedVariableValues::toInlinedTerm(const program::WhileStatement *whileStatement, std::shared_ptr<const program::Expression> expr, std::shared_ptr<const logic::Term> timepoint, std::shared_ptr<const logic::Term> trace) {
        ...
        if (typeid(*expr) == typeid(program::ArithmeticConstant)) {
            ...
            auto castedExpr = std::static_pointer_cast<const program::BooleanOr>(expr);
            return logic::Formulas::disjunction({toInlinedTerm(whileStatement, castedExpr->child1, timepoint, trace), toInlinedTerm(whileStatement, castedExpr->child2, timepoint, trace)});
        }
        else if (typeid(*expr) == typeid(program::BooleanNot)) {
            auto castedExpr = std::static_pointer_cast<const program::BooleanNot>(expr);
            return logic::Formulas::negation(toInlinedTerm(whileStatement, castedExpr->child, timepoint, trace));
        }
        else if (typeid(*expr) == typeid(program::ArithmeticComparison)) {
            auto castedExpr = std::static_pointer_cast<const program::ArithmeticComparison>(expr);
            switch (castedExpr->kind) {
