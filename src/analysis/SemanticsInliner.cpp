...
std::shared_ptr<const logic::Term> SemanticsInliner::toCachedTermFull(
  ...
  return logic::Terms::arraySelect(toTerm(arrayVar, cachedTimepoint, trace),
                                   position);
}
    ...
    if (varTermShouldBeDefined) {
      if (!var->isArray()) {
        ...
        auto f = logic::Formulas::equality(
            ...
            logic::Terms::arraySelect(
                toTerm(var, cachedArrayVarTimepoints[var], trace), pos));
        conjuncts.push_back(f);
      }
    }
  }
...
std::shared_ptr<const logic::Term> InlinedVariableValues::toInlinedTerm(
  ...
  return logic::Terms::arraySelect(toTerm(arrayVar, timepoint, trace),
                                   position);
}
