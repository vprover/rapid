...
namespace analysis {
    ...
    void ValueEvolutionLemmas::generateOutputForInteger(const program::WhileStatement* whileStatement,
                                                                  std::vector<std::shared_ptr<const logic::ProblemItem>>& items)
    {
        auto boundLSymbol = logic::Signature::varSymbol("boundL", logic::Sorts::intSort());
        auto boundRSymbol = logic::Signature::varSymbol("boundR", logic::Sorts::intSort());
        ...
        auto lStartboundR = timepointForLoopStatement(whileStatement, boundR);
        auto lStartIt = timepointForLoopStatement(whileStatement, it);
        auto lStartSuccOfIt = timepointForLoopStatement(whileStatement, logic::Theory::intSucc(it));
    ...
    void StaticAnalysisLemmas::generateOutputForInteger(const program::WhileStatement *statement, std::vector<std::shared_ptr<const logic::ProblemItem>>& items)
    {
        auto itSymbol = iteratorSymbol(statement);
        auto it = iteratorTermForLoop(statement);
            ...
            if (!v->isConstant && assignedVars.find(v) == assignedVars.end())
            {
                for (unsigned traceNumber = 1; traceNumber < numberOfTraces+1; traceNumber++)
                {
                    ...
                    auto lemma =
                        logic::Formulas::universal(argSymbols,
                            logic::Formulas::implication(
                                logic::Theory::intLessEqual(it, n),
                                inductionHypothesis(it)
                            )
                        );
