...
namespace analysis {
    ...
    void IntermediateValueLemmas::generateOutputForInteger(const program::WhileStatement *statement, std::vector<std::shared_ptr<const logic::ProblemItem>>& items){
        auto itSymbol = iteratorSymbol(statement);
        auto it = iteratorTermForLoop(statement);
        ...
        auto lStartIt = timepointForLoopStatement(statement, it);
        auto lStartIt2 = timepointForLoopStatement(statement, it2);
        auto lStartSuccOfIt = timepointForLoopStatement(statement, logic::Theory::intSucc(it));
        auto lStartSuccOfIt2 = timepointForLoopStatement(statement, logic::Theory::intSucc(it2));
                ...
                if (!v->isConstant && assignedVars.find(v) != assignedVars.end())
                {
                    if (!v->isArray) // We assume that loop counters are not array elements and therefore only add iterator-lemmas for non-array-vars
                    {
                        for (unsigned traceNumber = 1; traceNumber < numberOfTraces+1; traceNumber++)
                        {
                            ...
                            {
                                auto lStartArg = timepointForLoopStatement(statement, arg);
                                return logic::Theory::intLess(toTerm(v,lStartIt1,trace), toTerm(v,lStartArg,trace));
                            };
    ...
    void IterationInjectivityLemmas::generateOutputForInteger(const program::WhileStatement *statement, std::vector<std::shared_ptr<const logic::ProblemItem>> &items)
    {
        auto itSymbol = iteratorSymbol(statement);
        auto it = iteratorTermForLoop(statement);
        ...
        auto lStartSuccOfIt = timepointForLoopStatement(statement, logic::Theory::intSucc(it));
        auto lStartIt1 = timepointForLoopStatement(statement, it1);
        auto lStartSuccOfIt1 = timepointForLoopStatement(statement, logic::Theory::intSucc(it1));
        auto lStartIt2 = timepointForLoopStatement(statement, it2);
