      ...
      if (!v->isConstant && v->type() != program::ValueType::Bool && assignedVars.find(v) != assignedVars.end()) {
                          ...
                          // non-array-vars
        {
          for (unsigned traceNumber = 1; traceNumber < numberOfTraces + 1;
            ...
            auto dense = logic::Formulas::lemmaPredicate("Dense-" + nameSuffix,
                                                         freeVars1);
    ...
    if (!v->isConstant && v->type() != program::ValueType::Bool &&
                        ...
                        // non-array-vars
      {
        for (unsigned traceNumber = 1; traceNumber < numberOfTraces + 1;
          ...
          // Dense_v: forall it. (0≤it<n => ( v(l(s(it))    )=v(l(it)    ) or
          // v(l(s(it))    )=v(l(it)    )+1 ) )         , or
          //          forall it. (0≤itit<n => ( v(l(s(it)),pos)=v(l(it),pos) or
          //          v(l(s(it)),pos)=v(l(it),pos)+1 ) )
          auto denseFormula = logic::Formulas::universal(
              {itSymbol},
              logic::Formulas::implication(
          ...
          auto denseStrict = logic::Formulas::lemmaPredicate(
              "Dense-strict-" + nameSuffix, freeVars1);
