...
namespace analysis {
   ...
   void VariableCompLemmas::generateOutputFor(const program::WhileStatement *statement, std::vector<std::shared_ptr<const logic::ProblemItem>>& items)
    {
        if(util::Configuration::instance().integerIterations()){
            //TODO add for integers
            //AtLeastOneIterationLemmas::generateOutputForInteger(statement, items);
            ...
            //arbitrary trace, we are not really interested in traces here
            auto trace = traceTerm(0);
            auto n = lastIterationTermForLoop(statement, 0, trace);
            ...
            auto createInductionFormula = [&](bool lessEqual, auto var1, auto var2) { 
                
                std::function<std::shared_ptr<const logic::Formula>(std::shared_ptr<const logic::Term>, std::shared_ptr<const logic::Term>, std::string)> op;

                op = lessEqual ? logic::Theory::intLessEqual : logic::Theory::intGreaterEqual;

                bool sameVar = (var1 == var2);

                auto base =  op(toTerm(var1, lStartZero, trace), 
                    (sameVar ? toTerm(var1, lStartZero, trace) : 
                               toTerm(var2, lStartZero, trace)), "" );

                auto step1 = op(toTerm(var1, lStartIt, trace), 
                    (sameVar ? toTerm(var1, lStartZero, trace) : 
                               toTerm(var2, lStartIt, trace)), "");

                auto step2 = op(toTerm(var1, lStartSuccOfIt, trace), 
                    (sameVar ? toTerm(var1, lStartZero, trace) : 
                               toTerm(var2, lStartSuccOfIt, trace)), "");   
            ...
            for (const auto& var : assignedVars){
                if(!var->isConstant && !var->isArray){
                
                    //TODO add formulas for < and > as well?
                    auto indFormula1 = createInductionFormula(true, var, var);  
                    auto indFormula2 = createInductionFormula(false, var, var);

                    items.push_back(std::make_shared<logic::Lemma>(indFormula1, 
                        var->name + "-less-equal-induction-axiom", 
                        logic::ProblemItem::Visibility::Implicit));

                    items.push_back(std::make_shared<logic::Lemma>(indFormula2, 
                        var->name + "-greater-equal-induction-axiom", 
                        logic::ProblemItem::Visibility::Implicit));

                    /*for(unsigned i = index; i < assignedVars.size(); i++){
                        auto var2 = *std::next(assignedVars.begin(), i);
                        if(!var2->isConstant && !var2->isArray){
                            indFormula1 = createInductionFormula(true, var, var2);  
                            indFormula2 = createInductionFormula(false, var, var2);
                            
                            items.push_back(std::make_shared<logic::Lemma>(indFormula1, 
                                var->name + "-" + var2->name + "-less-equal-induction-axiom", 
                                logic::ProblemItem::Visibility::Implicit));

                            items.push_back(std::make_shared<logic::Lemma>(indFormula2, 
                                var->name + "-" + var2->name + "-greater-equal-induction-axiom", 
                                logic::ProblemItem::Visibility::Implicit));                           
                        }
                    }*/
                }
