...
namespace analysis {
    ...
    std::shared_ptr<const logic::Term> Semantics::generateSemantics(const program::Assignment *assignment, SemanticsInliner &inliner, std::shared_ptr<const logic::Term> trace) {
        ...
        if (typeid(*assignment->lhs) == typeid(program::VariableAccess)) {
            ...
            if (util::Configuration::instance().inlineSemantics()) {
                ...
                auto f2 = inliner.setVarValue(castedLhs->var, inliner.toCachedTerm(assignment->rhs));
                ...
                // lhs(l2) = rhs(l1)
                auto eq = logic::Formulas::equality(toTerm(castedLhs, l2, trace), toTerm(assignment->rhs, l1, trace));
                ...
                return logic::Formulas::conjunction(conjuncts, "Update variable " + castedLhs->var->name + " at location " + assignment->location);
            }
        }
    ...
    std::shared_ptr<const logic::Term> Semantics::generateSemantics(const program::WhileStatement *whileStatement, SemanticsInliner &inliner, std::shared_ptr<const logic::Term> trace) {
        ...
        if (util::Configuration::instance().inlineSemantics()) {
            ...
            for (const auto &var : loopConditionVars) {
                if (assignedVars.find(var) == assignedVars.end()) {
                    if (var->isArray) {
                        ...
                        inlinedVariableValues.setValue(whileStatement, var, trace, inliner.getCachedVarValues().at(var));
                    }
                }
            }
            ...
            auto negConditionAtN = logic::Formulas::negation(toTerm(whileStatement->condition, lStartN, trace), "The loop-condition doesn't hold in the last iteration");
