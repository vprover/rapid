...
void EqualityPreservationTracesLemmas::generateOutputFor(
  ...
  if (util::Configuration::instance().integerIterations()) {
    EqualityPreservationTracesLemmas::generateOutputForInteger(statement,
                                                               items);
  } else {
      ...
      if (!v->isConstant && assignedVars.find(v) != assignedVars.end()) {
        for (unsigned traceNumber1 = 1; traceNumber1 < numberOfTraces + 1;
          ...
          for (unsigned traceNumber2 = traceNumber1 + 1;
               traceNumber2 < numberOfTraces + 1; traceNumber2++) {
            auto t1 = traceTerm(traceNumber1);
            auto t2 = traceTerm(traceNumber2);

            // Note: We use the induction axiom directly as lemma, so the lemma
            // trivially holds and we don't need to prove it.
            auto nameSuffix = "-" + v->name + "-" + statement->location + "-" +
                              std::to_string(traceNumber1) +
                              std::to_string(traceNumber2);
            ...
            auto inductionHypothesis =
                [&](std::shared_ptr<const logic::Term> arg) {
                  ...
                  return logic::Formulas::equality(toTerm(v, lStartArg, t1),
  ...
  if (util::Configuration::instance().integerIterations()) {
    ...
    for (unsigned traceNumber1 = 1; traceNumber1 < numberOfTraces + 1;
      ...
      for (unsigned traceNumber2 = traceNumber1 + 1;
            ...
            if ((inlineSemantics &&
              ...
              if (v->isArray()) {/*
                ...
                conjuncts.push_back(logic::Formulas::universal(
                    {posSymbol},
                    logic::Formulas::equality(toTerm(v, lStartArg, pos, t1),
                                              toTerm(v, lStartArg, pos, t2))));*/
              } else {
                conjuncts.push_back(logic::Formulas::equality(
                    toTerm(v, lStartArg, t1), toTerm(v, lStartArg, t2)));
              }
            }
          }
        ...
        auto [inductionAxBCDef, inductionAxICDef, inductionAxiomConDef,
              inductionAxiom] =
            logic::inductionAxiom2(inductionAxiomName, inductionAxiomNameShort,
                                   inductionHypothesis, nT1, nT2,
                                   freeVarSymbols);
        items.push_back(inductionAxBCDef);
        items.push_back(inductionAxICDef);
        items.push_back(inductionAxiomConDef);
        ...
        for (const auto& v : loopConditionVars) {
          if ((inlineSemantics && assignedVars.find(v) == assignedVars.end()) ||
            ...
            if (v->isArray()) {/*
              auto posSymbol = posVarSymbol();
              auto pos = posVar();
              premiseConjuncts.push_back(logic::Formulas::universal(
                  {posSymbol},
                  logic::Formulas::equality(
                      inlineSemantics ? inlinedVarValues.toInlinedTerm(
                                            statement, v, pos, t1)
              ...
              premiseConjuncts.push_back(logic::Formulas::equality(
                  inlineSemantics
        ...
        // PART 2B: Add IH(zero) to premise, ensuring that non-constant vars are
        // the same at the beginning of the loop execution
        ...
        auto inductiveCase =
            logic::Formulas::lemmaPredicate("IC-" + nameShort, freeVars);
        ...
        //          IH(it)
        //       IH(s(it))
        auto inductiveCaseFormula = logic::Formulas::universal(
            {itSymbol},
            logic::Formulas::implication(
                logic::Formulas::conjunction({logic::Theory::natSub(it, nT1),
        ...
        auto inductiveCaseDef = std::make_shared<logic::Definition>(
            logic::Formulas::universal(
                freeVarSymbols, logic::Formulas::equivalence(
                                    inductiveCase, inductiveCaseFormula)),
            "IC for " + name, logic::ProblemItem::Visibility::Implicit);
        ...
        auto lemma = logic::Formulas::universal(
            freeVarSymbols, logic::Formulas::implication(
  ...
  for (unsigned traceNumber1 = 1; traceNumber1 < numberOfTraces + 1;
    ...
    for (unsigned traceNumber2 = traceNumber1 + 1;
      ...
      auto [inductionAxBCDef, inductionAxICDef, inductionAxiomConDef,
            inductionAxiom] =
          logic::inductionAxiom2(inductionAxiomName, inductionAxiomNameShort,
                                 inductionHypothesis, nT1, nT2, freeVarSymbols);
      items.push_back(inductionAxBCDef);
      items.push_back(inductionAxICDef);
      items.push_back(inductionAxiomConDef);
      items.push_back(inductionAxiom);
      ...
      // PART 2B: Add IH(zero) to premise, ensuring that non-constant vars are
      // the same at the beginning of the loop execution
      premiseConjuncts.push_back(inductionHypothesis(logic::Theory::zero()));
