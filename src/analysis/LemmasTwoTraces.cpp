          ...
          auto inductionHypothesis =
              [&](std::shared_ptr<const logic::Term> arg) {
                ...
                return logic::Formulas::equality(
                    /*v->isArray() ? toTerm(v, lStartArg, pos, t1) :*/ toTerm(
                        v, lStartArg, t1),
                    /*v->isArray() ? toTerm(v, lStartArg, pos, t2) :*/ toTerm(
                        v, lStartArg, t2));
              };
...
void NEqualLemmas::generateOutputFor(
  ...
  for (unsigned traceNumber1 = 1; traceNumber1 < numberOfTraces + 1;
    ...
    for (unsigned traceNumber2 = traceNumber1 + 1;
            ...
            /*  auto posSymbol = posVarSymbol();
              ...
              premiseConjuncts.push_back(
                  logic::Formulas::universal(
                      ...
                      logic::Formulas::equality(
                          inlineSemantics ?
              inlinedVarValues.toInlinedTerm(statement, v, pos, t1) : toTerm(v,
              nullptr, pos, t1), inlineSemantics ?
              inlinedVarValues.toInlinedTerm(statement, v, pos, t2) : toTerm(v,
              nullptr, pos, t2)
                      )
                  )
              );*/
      ...
      premiseConjuncts.push_back(inductionHypothesis(logic::Theory::natZero()));
