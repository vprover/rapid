...
namespace analysis {
    ...
    void EqualityPreservationTracesLemmas::generateOutputForInteger(const program::WhileStatement *statement, std::vector<std::shared_ptr<const logic::ProblemItem>>& items)
    {
        auto posSymbol = posVarSymbol();
        auto pos = posVar();
    ...
    void NEqualLemmas::generateOutputForInteger(const program::WhileStatement *statement, std::vector<std::shared_ptr<const logic::ProblemItem>>& items)
    {
        assert(numberOfTraces > 1);
        ...
        for (unsigned traceNumber1 = 1; traceNumber1 < numberOfTraces+1; traceNumber1++)
        {
            for (unsigned traceNumber2 = traceNumber1+1; traceNumber2 < numberOfTraces+1; traceNumber2++)
            {
                ...
                // PART 2B: Add IH(zero) to premise, ensuring that non-constant vars are the same at the beginning of the loop execution
                premiseConjuncts.push_back(
                    inductionHypothesis(logic::Theory::zero())
                );
