...
int main(int argc, char *argv[]) {
  ...
  for (const auto &dirEntry : std::filesystem::recursive_directory_iterator(testDir)) {
    ...
    if (dirEntry.path().extension().string() != ".spec"
        || dirEntry.path().string().find("experiments") != std::string::npos) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      char *params[] = {(char *) "",
                        (char *) "-overwriteExisting", (char *) "on",
                        (char *) "-inlineSemantics", (char *) ((i & 1) == 0 ? "on" : "off"),
                        (char *) "-nativeArrays", (char *) ((i & 3) == 0 ? "on" : "off"),
                        (char *) "-dir", &output[0], &input[0]};
