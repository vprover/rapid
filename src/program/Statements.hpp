...
class Statement {
 public:
  Statement(unsigned lineNumber)
      : location("l" + std::to_string(lineNumber)),
...
class IntAssignment : public Statement {
 public:
  IntAssignment(unsigned lineNumber, std::shared_ptr<const IntExpression> lhs,
                std::shared_ptr<const IntExpression> rhs)
...
class IfElse : public Statement {
 public:
  IfElse(unsigned lineNumber, std::shared_ptr<const BoolExpression> condition,
         std::vector<std::shared_ptr<const Statement>> ifStatements,
         std::vector<std::shared_ptr<const Statement>> elseStatements)
...
class WhileStatement : public Statement {
 public:
  WhileStatement(unsigned lineNumber,
                 std::shared_ptr<const BoolExpression> condition,
                 std::vector<std::shared_ptr<const Statement>> bodyStatements)
      : Statement(lineNumber),
        condition(std::move(condition)),
