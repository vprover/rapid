...
class Formula {
 public:
  ...
  virtual std::string toSMTLIB(unsigned indentation = 0,
                               bool singleLine = false) const = 0;
...
class PredicateFormula : public Formula {
 ...
 public:
  PredicateFormula(std::shared_ptr<const Symbol> symbol,
                   ...
                   std::string label = "")
      : Formula(label), symbol(symbol), subterms(subterms) {
  ...
  std::string toSMTLIB(unsigned indentation = 0,
                       bool singleLine = false) const override;
...
class ConjunctionFormula : public Formula {
 ...
 public:
  ConjunctionFormula(std::vector<std::shared_ptr<const Formula>> conj,
                     std::string label = "")
      : Formula(label), conj(conj) {}
...
class DisjunctionFormula : public Formula {
 ...
 public:
  DisjunctionFormula(std::vector<std::shared_ptr<const Formula>> disj,
                     std::string label = "")
      : Formula(label), disj(disj) {}
...
class NegationFormula : public Formula {
 ...
 public:
  NegationFormula(std::shared_ptr<const Formula> f, std::string label = "")
      : Formula(label), f(f) {}
...
class ImplicationFormula : public Formula {
 ...
 public:
  ImplicationFormula(std::shared_ptr<const Formula> f1,
                     std::shared_ptr<const Formula> f2, std::string label = "")
...
class EquivalenceFormula : public Formula {
 ...
 public:
  EquivalenceFormula(std::shared_ptr<const Formula> f1,
                     std::shared_ptr<const Formula> f2, std::string label = "")
      : Formula(label), f1(f1), f2(f2) {}
