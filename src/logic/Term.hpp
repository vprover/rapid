...
class Term {
 public:
  ...
  Term(std::shared_ptr<const Symbol> symbol, std::string label)
      : symbol(symbol), hasSymbol(true), label(label) {}
 ...
 protected:
  std::string stringForLabel(unsigned indentation = 0) const;
  std::string stringForLabelTPTP(unsigned indentation = 0) const;
};
...
class LVariable : public Term {
  ...
  LVariable(std::shared_ptr<const Symbol> symbol)
      : Term(symbol), id(freshId++) {}
 ...
 public:
  ...
  virtual std::string prettyString(unsigned indentation = 0) const override;
...
class FuncTerm : public Term {
 ...
 protected:
  FuncTerm(std::shared_ptr<const Symbol> symbol,
           std::vector<std::shared_ptr<const Term>> subterms, std::string label)
      : Term(std::move(symbol), label), subterms(std::move(subterms)) {
 ...
 public:
  ...
  std::string toSMTLIB(unsigned indentation = 0) const override;
  virtual std::string toTPTP(unsigned indentation = 0) const override;
  virtual std::string prettyString(unsigned indentation = 0) const override;
};
