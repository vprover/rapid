...
namespace logic {
    ...
    std::vector<ReasoningTask> Problem::generateReasoningTasks() const
    {
        ...
        for (int i = 0; i < items.size(); ++i)
        {
            ...
            if(util::Configuration::instance().postcondition() && item->type == ProblemItem::Type::Conjecture){
                // generate output for each conjecture in a seperate file to be used to verify which 
                // invariants generated by symbol elimination are used to proof the conjecture
            }
